		...
		[] spawn
		{
			while {true} do
			{
				...
				switch (_ranknumber) do  // Apply AJ Rankinsignia
					{
					case 9: {[player,"aj_rank_BrigGen"] call BIS_fnc_setUnitInsignia; };  // General
					case 8: {[player,"aj_rank_OTL"] call BIS_fnc_setUnitInsignia; }; 	  // Coronel Core Mission Developer
					case 5: {[player,"aj_rank_Hptm"] call BIS_fnc_setUnitInsignia; };     // Squad Leader  - AJ Squad Leaders
					case 3: {[player,"aj_rank_HG"] call BIS_fnc_setUnitInsignia; };       // Soldier - AJ Default
					case 1: {[player,"aj_rank_gef"] call BIS_fnc_setUnitInsignia; };      // Noobsoldier - AJ Trials
					default {};
					};
				...
				if ( _uniformNumber ==  10 && (headgear player) == "H_MilCap_dgtl") then // 10 = armajunkies uniform
					...
					case 5: { player addHeadgear "H_Bandanna_mcamo"; };   // Hauptmann  - AJ Squad Leaders
