...
_setupObjects =
{
	...
	[
		...
		["O_Boat_Armed_01_hmg_F", "B_Heli_Transport_03_F"],
		["I_Boat_Armed_01_minigun_F", "I_Heli_light_03_F"]
	];

	if (missionDifficultyHard) then
	{
		(_vehChoices select 0) set [1, "B_Heli_Light_01_armed_F"];
		(_vehChoices select 1) set [1, "I_Heli_light_03_F"];
		(_vehChoices select 2) set [1, "O_Heli_Light_02_v2_F"];
	};
...
_successExec =
{
	...
	[_box2, ["mission_USLaunchers", "mission_AJ_Gear1", "mission_AJ_Sniper2"] call BIS_fnc_selectRandom] call fn_refillbox;

	_box3 = createVehicle ["Box_IND_WpsSpecial_F", _lastPos, [], 5, "None"];
	_box3 setDir random 360;
	[_box3, ["mission_USLaunchers", "mission_AJ_Gear1"] call BIS_fnc_selectRandom] call fn_refillbox;
